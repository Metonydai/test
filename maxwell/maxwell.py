...
core_dim_dict = {}
for cdim in receive_dict['core']['dims']:
    core_dim_dict[cdim['name']] = cdim['value']

slice_x = 0.0
slice_y = core_dim_dict['Dim_F'] / 4
slice_z = (core_dim_dict["Dim_D"] + core_dim_dict['Dim_E']) / 2
print("slice x, y, z", slice_x, slice_y, slice_z)

plots_dir = Path(receive_dict["magneticFieldPlotFileDirectory"])
...
for i, time in enumerate(vari_list):
    ...
    imgs = {
        "timePoint": ftime,
        "imgXY": str(plots_dir / f"xy_{i}.jpg"),
        "imgZX": str(plots_dir / f"zx_{i}.jpg"),
        "imgYZ": str(plots_dir / f"yz_{i}.jpg"),
        "vtk3D": str(plots_dir / f"3d_{i}.vtksz"),
        "centerLeg": 0.0,
        "sideLeg": 0.0,
        "topPlate": 0.0,
        "bottomPlate": 0.0
